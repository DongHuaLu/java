...
public class JMineMatrixPanel extends JPanel {
	...
	private class BlockMouseListener extends MouseAdapter {
		...
		public void mouseEntered(MouseEvent e) {
			...
			if (button1Pressed && button3Pressed) {
				...
				if (button1Pressed) {// 在鼠标左键按下的条件下

					if (isFlagged(row, column)) {//已标记 中断
						return;
					}
					if (isDigged(row, column)) {//已挖 中断
						return;
					} else {// 若没被挖则 显示空白
		...
		public void mouseExited(MouseEvent e) {
			...
			if (button3Pressed && button1Pressed) {// 两键同时被按下的情况Exit
				// 显示出原图
			...
			if (button3Pressed) {// 右键被按下
				currentMineBlock = null;
			} else {// 右键未被按下 更新行列
				int row = currentMineBlock.getRow();
				int column = currentMineBlock.getColumn();

				if (button1Pressed) {// 左键按下 显示原图
					if (isFlagged(row, column)) {
					} else if (!isDigged(row, column)) {

						if (isRabotOn() && isMine(row, column)) {
							currentMineBlock.setDot();//!Flagged&&!isDigged
						} else {
		...
		public void mousePressed(MouseEvent e) {
			...
			int column = currentMineBlock.getColumn();
			// 左右键都按下，则探测
			if (button1Pressed && button3Pressed) {// 两键都被按下 自动探测
				mineSweeperFrame.leftButtonPressedOnMineBlock();
				setAutoProbeIcons(row, column);
				} else if (button1Pressed && !button3Pressed) {// 左键按下，右键未按下
				mineSweeperFrame.leftButtonPressedOnMineBlock();
				if (isDigged(row, column)) {
					return;
				} else if (isFlagged(row, column)) {// 被置旗则也不变
					return;
				}
			...
			{// 右键被按下 左键未按下
				...
				mineSweeperFrame.setLEDMineCountLeft(mineMatrix.getMineCount()
						- mineMatrix.getFlagCount());
				refreshMineMatrixPanel();
				return;
			}
		 ...
		 */
		@Override
		public void mouseReleased(MouseEvent e) {// 它的源是鼠标按下时的组件。
			...
			if ((button1Released && button3Pressed)// 就是进行探测了。
					|| (button1Pressed && button3Released)) {// 右键按下时 左键放开
				// 或者左键按下时右键放开
				...
				// 恢复图形显示
				revertAutoProbeIcons(row, column);
				if (!isDigged(row, column)) {// 如果没被挖过就不能自动探测
					return;
				}
				mineMatrix.autoDetectMine(currentMineBlock.getRow(),
						currentMineBlock.getColumn());
			...
			// 左键放开
			// 挖雷 若已被挖开则不挖 刷新一次界面
			else if (button1Released && !button3Pressed) {// 右键未按下时左键放开挖雷
				...
				int row = currentMineBlock.getRow();
				int column = currentMineBlock.getColumn();
				if (mineMatrix.isDigged(row, column)) {// 判断是否已经被挖开
					return;
				} else {// 没挖开就挖
					// 还要判断是否是第一挖
					if (mineMatrix.isFlagged(row, column)) {// 要先判断是否放旗。
						// 是旗则不挖
						return;
					}
					if (!mineMatrix.hasGameCommenced()) {// 如果未开局
					...
					if (isMine(row, column)) {// 如果挖的是雷则释放所有监听器
	 ...
	 *            雷块的列号
	 */
	private void setAutoProbeIcons(int row, int column) {// 自动探测时的图标显示
		// 将周围被标记的置为标记按下
		for (int i = -1; i < 2; i++) {
			for (int j = -1; j < 2; j++) {
	 ...
	 *            雷块的列号
	 */
	private void revertAutoProbeIcons(int row, int column) {// 自动探测时的图标显示
		// 将周围被标记的置为标记按下
		for (int i = -1; i < 2; i++) {
			for (int j = -1; j < 2; j++) {
	 ...
	 *            雷块的列号
	 */
	private void setAutoProbeIcon(int row, int column) {// 自动探测时的图标显示
		if (row < 0 || row > this.getMatrixRowCount() - 1 || column < 0
				|| column > this.getMatrixColumnCount() - 1) {
			return;
		}
		// 将周围被标记的置为标记按下
	 ...
	 *            雷块的列号
	 */
	private void revertAutoProbeIcon(int row, int column) {// 自动探测时的图标显示
		if (row < 0 || row > this.getMatrixRowCount() - 1 || column < 0
				|| column > this.getMatrixColumnCount() - 1) {
			return;
		}
	...
	public JMineMatrixPanel() {
		...
		Border b1 = BorderFactory.createCompoundBorder(new BevelBorder(
				BevelBorder.LOWERED), new BevelBorder(BevelBorder.LOWERED));
		Border b2 = BorderFactory.createCompoundBorder(new EmptyBorder(5, 5, 5,
				5), b1);
		setBorder(b2);
	...
	private void gameTerminates() {
		...
		for (int row = 0; row < this.getMatrixRowCount(); row++) {
			for (int column = 0; column < this.getMatrixColumnCount(); column++) {
				jMineMatrix[row][column]
						.removeMouseListener(blockMouseListener);
	...
	public void refreshMineMatrixPanel() {
		...
		for (int r = 0; r < row; r++) {
			for (int c = 0; c < column; c++) {
				if (this.isDigged(r, c)) {// 被挖过
					if (this.isMine(r, c)) {// 是雷
						// 被挖是雷 则显示出红雷
				...
				} else {// 没被挖过则 显示是否放置旗 和标记

					if (isFlagged(r, c) && !isMine(r, c)
							&& mineMatrix.isGameOver()) {// 没被挖过 被置旗 不是雷
						jMineMatrix[r][c].setWrongMine();
						continue;
					}
					if (!isFlagged(r, c) && isMine(r, c)
							&& mineMatrix.isGameOver()) {// 游戏结束是雷而且没被置旗
						jMineMatrix[r][c].setMine();
						continue;
					}
					if (isFlagged(r, c)) {// 未挖过，是旗
						jMineMatrix[r][c].setFlag();
						continue;
					}
					if (isMarked(r, c)) {// 没被挖过 被置标记
						jMineMatrix[r][c].setAsk();
						continue;

					}
					if (isRabotOn() && isMine(r, c)) {// 开启作弊而且又是雷
	 ...
	 * 
	 */
	private void resetMineMatrixLabel() {// 重置雷阵的图标数量。
		int row = this.getMatrixRowCount();
		int column = this.getMatrixColumnCount();
		...
		for (int r = 0; r < row; r++) {
			for (int c = 0; c < column; c++) {
				this.jMineMatrix[r][c] = new JMineBlock(ImageIconFactory
						.getBlank());
				jMineMatrix[r][c].setRow(r);
				jMineMatrix[r][c].setColumn(c);
				this.jMineMatrix[r][c]
						.addMouseListener(this.blockMouseListener);
