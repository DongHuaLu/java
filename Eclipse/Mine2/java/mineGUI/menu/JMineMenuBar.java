...
import mineGUI.dialog.JCopyRightDialog;
import mineGUI.dialog.JRankDialog;
import mineGUI.dialog.SelfDefinedGameModeDialog;
import mine_model.Strategy.AdvancedLevel;
import mine_model.Strategy.IntermediateLevel;
import mine_model.Strategy.PreliminaryLevel;
import mine_model.Strategy.SelfDefinedLevel;
 ...
 */
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == gameMenu.commence) {// 开始游戏
			mineFrame.resetGame();
			return;
		}
		if (e.getSource() == gameMenu.preliminaryLevel) {// 选择初级
			mineFrame.setGameMode(new PreliminaryLevel());
			return;
		}
		if (e.getSource() == gameMenu.intermediateLevel) {// 选择中级
			mineFrame.setGameMode(new IntermediateLevel());
			return;
		}
		if (e.getSource() == gameMenu.advancedLevel) {// 选择高级
			mineFrame.setGameMode(new AdvancedLevel());
			return;
		}
		if (e.getSource() == gameMenu.selfDefinedLevel) {// 选择自定义级
			int row = mineFrame.getRowCount();// 得到当前程序的游戏模式数据
			int column = mineFrame.getColumnCount();
			int mine = mineFrame.getMineCount();
			SelfDefinedGameModeDialog dialog = new SelfDefinedGameModeDialog(
					mineFrame, row, column, mine);
			dialog.setLocationRelativeTo(mineFrame);
			if (dialog.showDialog() == SelfDefinedGameModeDialog.APPROVE_OPTION) {
				mineFrame.setGameMode(new SelfDefinedLevel(
						dialog.getRowCount(), dialog.getColumnCount(), dialog
								.getMineCount()));
			}
			return;
		}
		if (e.getSource() == gameMenu.markable) {// 选择标记
			if (gameMenu.markable.isSelected()) {// 被选中后会自动设置为没选中。这是单击后新的状态而不是原来的状态。
		...
		if (e.getSource() == helpMenu.copywright) {// 选择版权 
