...
public class MineMatrix {
	...
	private void addMineCountByOne(int row, int column) {
		if (row < 0 || row > this.row - 1 || column < 0
	 ...
	 *            列号
	 */
	public void autoDetectMine(int row, int column) {// 自动探测
		if (row < 0 || column < 0 || row > this.row - 1
				|| column > this.column - 1) {
		}
		...
		if (getMineCountAround(row, column) == getFlagCountAround(row, column)) {// 自动挖雷
			// 扫描附近没有被挖过的雷，然后挖雷。
			for (int i = -1; i < 2; i++) {
				for (int j = -1; j < 2; j++) {
					if (!isDiggedAutoCorrect(row + i, column + j)// 未被挖过
							&& !isFlaggedAutoCorrect(row + i, column + j)) {// 未被置过旗
	 ...
	 *            列号
	 */
	protected void autoDigMine(int row, int column) {// 自动挖雷的条件是什么？1.被标记过。2.未被自动探测过。
	...
	public void digBlock(int row, int column) {
		if (isMine(row, column)) {// 如果是雷则将gameOver设置为true
	...
	public void generateRandomMatrix(int clickedRow, int clickedColumn) {
		...
		// 生成随机雷阵时也把标记置为false
		gameCommenced = false;
		if (clickedRow < 0 || clickedColumn < 0 || clickedRow > this.row - 1
				|| clickedColumn > this.column - 1) {
	...
	private boolean isDiggedAutoCorrect(int row, int column) {
		if (row < 0 || column > this.column - 1 || column < 0
				|| row > this.row - 1) {
			return false;
		}
