...
public class JMineSweeperFrame extends JFrame implements WindowListener {
	...
	public List<Rank> preliminaryLevelRanks = new LinkedList<Rank>();//链表
	public List<Rank> intermediateLevelRanks = new LinkedList<Rank>();//链表
	public List<Rank> advancedLevelRanks = new LinkedList<Rank>();//链表
	...
	public JMineSweeperFrame() {
		...
		this.setIconImage(new ImageIcon("./image/icon.gif").getImage());//图标
		this.setTitle("扫雷");//title:扫雷
		this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);//关闭方式
		this.setJMenuBar(menuBar);//设置窗体的菜单栏
		this.add(statusPanel, BorderLayout.NORTH);//添加状态Panel
		this.add(jMineMatrixPanel);//添加matrixPanel
	
		jMineMatrixPanel.addMineSweeperFrame(this);
		statusPanel.addMineSweeperFrame(this);
		setGameMode(new PreliminaryLevel());//设置游戏模式为初级
		this.addWindowListener(this);
		menuBar.setMineSweeperFrame(this);
		this.setResizable(false);//设置用户不可调整窗口大小
	...
	public void gameComplete() {// 完成游戏
		...
		if (getGameModeName().equals("PreliminaryLevel")) {// 如果是完成初级
			String name = getHeorName();
			preliminaryLevelRanks.add(new Rank("初级", time, name));
		} else if (getGameModeName().equals("IntermediateLevel")) {// 如果是完成中级
			String name = getHeorName();
			intermediateLevelRanks.add(new Rank("中级", time, name));
		} else if (getGameModeName().equals("AdvancedLevel")) {// 如果是完成高级
			String name = getHeorName();
	 ...
	 *            游戏模式对象
	 */
	public void setGameMode(MineMatrixSettable s) {// 设置游戏模式
