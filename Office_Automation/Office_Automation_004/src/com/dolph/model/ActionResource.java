...
public class ActionResource implements SysResource {
	...
	public int[] getOperIndexs() {
		if(opers!=null){
			Collection<ActionMethodOper> actionMethodOpers=opers.values();
			int [] operIndexs=new int[actionMethodOpers.size()];
			int i=0;
			for(ActionMethodOper o:actionMethodOpers){
				operIndexs[i++]=o.getOperIndex();
			}
			return operIndexs;
		}
		return null;
	...
	public String getOperSnByMethodName(String methodName) {
		...
		for(ActionMethodOper amo:opers.values()){
			if(contain(amo.getMethodName(),methodName)){
	...
	private boolean contain(String dataMethodName, String paramMethodName) {
		String [] allMethodNames=dataMethodName.split("\\|");
		for(String m:allMethodNames){
			if(m.equals(paramMethodName)){
			return true;
			}
		}
		return false;
